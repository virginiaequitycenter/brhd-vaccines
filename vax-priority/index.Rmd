...
load("../assignment1_tractrankings/geo_rankings_indigenous.Rdata")
...
tract_dimensions %>%
  ...
  row_spec(1:4, background = brewer.pal(8, "Purples")[7]) %>% 
  #row_spec(5:17, background = brewer.pal(8, "Purples")[6]) %>% 
  column_spec(1:3, color = "white")
```

* Among the 49 census tracts within the Blue Ridge Health District localities (Albemarle, Charlottesville,[^1] Fluvanna, Greene, Louisa, and Nelson), four score especially high on the composite, with values of 3 or above;

* An additional 13 census tracts are present in the next highest range, 2 to 3; 

* Of the remaining census tracts, the majority (26) have composite scores between 1 and 2, while six are in the lowest range, less than 1, indicating the lowest level of risk.[^2]

The map visualizes the composite score values for all census tracts to ease identification of census tract locations.[^3] Clicking on the census tract will bring up the census tract name and composite value. A full table of sortable and searchable results is below. 
...
The following table provides the overall index, scoring tracts by the percent of residents who are Black, who are Native Amerian or Indigenous, who are Hispanic or Latinx, and who are experiencing poverty along with the average life expectancy of residents within the tract[^4] and the overall prevalance of health outcomes identified by CDC as risk factors: cancer (except skin cancer), chronic kidney disease, chronic obstructive pulmonary disease, diabetes, obesity, and smoking.[^5] 
...
Sam Powers completed the data extraction, preparation, and statistical analysis to evaluate the combination of these indicators into a composite meaure. Sam Powers and Michele Claibourn generated this brief report in consultation with Guleer Shahab.

[^1]: Census Tract 109.03 in Albemarle County was excluded. This area is predominantly UVA housing (McCormick Road housing, Lawn housing) and grounds and population estimates are either very volatile or missing for this tract. 

[^2]: The break values of 1 and 2 are a convenience -- the appropriate threshold for grouping tracts into the levels of risk is a decision requiring greater input and discussion.

[^3]: With a list of addresses for current vaccination distribution sites, we could add these to the map for easier visualization.

[^4]: Life expectancy is reversed before inclusion in the scale such that lower life expectancy values are ranked higher.

[^5]: Health outcomes are combined into a single index before inclusion in the overall composite score.
